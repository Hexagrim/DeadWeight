...
public class InteractableWater : MonoBehaviour
{
    ...
    public void GenerateMesh()
    {
        ...
        // vertices thing
        _vertices = new Vector3[NumerOfxVertices * NUM_OF_Y_VERTICES];
        _topVerticesIndex = new int[NumerOfxVertices];

        for (int y = 0; y < NUM_OF_Y_VERTICES; y++)
        {
            for (int x = 0; x < NumerOfxVertices; x++)
            {
                float xPos = (x / (float)(NumerOfxVertices - 1)) * Width - Width / 2;
                float yPos = (y / (float)(NUM_OF_Y_VERTICES - 1)) * Height - Height / 2;

                _vertices[y * NumerOfxVertices + x] = new Vector3(xPos, yPos, 0f);

                if (y == NUM_OF_Y_VERTICES - 1)
                {
                    _topVerticesIndex[x] = y * NumerOfxVertices + x;
                }
            }
        }

        //triangles
        int[] triangles = new int[(NumerOfxVertices - 1) * (NUM_OF_Y_VERTICES - 1) * 6];
        int index = 0;

        for (int y = 0; y < NUM_OF_Y_VERTICES - 1; y++)
        {
            for (int x = 0; x < NumerOfxVertices - 1; x++)
            {
                int bottomLeft = y * NumerOfxVertices + x;
                int bottomRight = bottomLeft + 1 ;
                int topLeft = bottomLeft + NumerOfxVertices;
...
public class InteractableWaterEditor : Editor
{
    ...
    public override VisualElement CreateInspectorGUI()
    {
        ...
        Button placeEdgeCooliderButton = new Button(() => _water.ResetEdgeCollider())
        {
            text = "Place Edge Collider"
        };

        root.Add(placeEdgeCooliderButton);
    ...
    private void OnSceneGUI()
    {
        ...
        Vector3 newBottomLeft = Handles.FreeMoveHandle(
            corners[0],
            handleSize,
            snap,
        ...
        EditorGUI.BeginChangeCheck();
        Vector3 newBottomRight = Handles.FreeMoveHandle(
            corners[1],
            handleSize,
            snap,
            Handles.CubeHandleCap
        ...
        EditorGUI.BeginChangeCheck();
        Vector3 newTopLeft = Handles.FreeMoveHandle(
            corners[2],
            handleSize,
            snap,
            Handles.CubeHandleCap
        ...
        EditorGUI.BeginChangeCheck();
        Vector3 newTopRight = Handles.FreeMoveHandle(
            corners[3],
            handleSize,
            snap,
            Handles.CubeHandleCap
